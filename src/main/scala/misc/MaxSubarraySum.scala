

package misc

object MaxSubarraySum {
  /*
	 Maximum subarray problem
	 
	 Kadane's algorithm
	 
	 Simple idea of the Kadane's algorithm is to look for all positive contiguous
	 segments of the array (max_ending_here is used for this). 
	 
	 And keep track of maximum sum contiguous segment among all positive segments 
	 (max_so_far is used for this). 
	 
	 Each time we get a positive sum compare it with max_so_far and update max_so_far 
	 if it is greater than max_so_far
	*/

  def main(args: Array[String]): Unit = {
    //val input: String = "7 -3 -10 4 2 8 -2 4 -5 -6" //16
    val input: String = "-2 1 -3 4 -1 2 1 -5 4" //6
    val numArray: Array[Int] = input.split(" ").map(_.toInt)

    var max_ending_here = 0
    var max_so_far = 0

    val tot = (for (x <- numArray) yield {
      max_ending_here = Math.max(0, max_ending_here + x)
      max_so_far = Math.max(max_ending_here, max_so_far)
      max_so_far
    }).max
    println(" Maximum sum =" + tot)
  }
}